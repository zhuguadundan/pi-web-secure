...
interface Props {
  ...
  busy?: boolean;
  error?: string | null;
  /** Pinned entries rendered above the directory list (e.g. home/code/work/super). */
  shortcuts?: Array<{ label: string; path: string }>;
}

/**
 * Modal filesystem browser for picking a workspace directory. Navigates via
 * GET /api/cwd/browse; selection is only handed to the caller — validation and
 * allowed-root registration happen in POST /api/cwd/validate.
 *
 * When the server disables browsing (PI_WEB_DISABLE_CWD_BROWSE=1) the picker
 * degrades to manual full-path entry so opening a known directory still works
 * without exposing filesystem enumeration.
 */
export function DirectoryPicker({ onCancel, onSelect, initialPath, busy = false, error, shortcuts = [] }: Props) {
  ...
  const hasUncommittedPath = !manualMode && pathInput.trim() !== currentPath;
  const canSelect = manualMode
    ? Boolean(pathInput.trim()) && !busy
    : Boolean(currentPath) && !hasUncommittedPath && !busy;
    ...
    >
      <div className="directory-picker-panel" style={{ width: 520, maxWidth: "calc(100vw - 16px)", height: "min(620px, calc(100dvh - 16px))", maxHeight: "calc(100dvh - 16px)", display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 32px rgba(0,0,0,0.18)" }}>
        ...
        <form onSubmit={handlePathSubmit} style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, padding: "10px 14px", borderBottom: "1px solid var(--border)" }}>
          {!manualMode && (
            <button className="directory-picker-back" type="button" onClick={() => void navigateTo(parentDirectory ?? undefined)} disabled={loading || !canNavigateUp} title="Go to parent directory" aria-label="Go to parent directory" style={{ width: 36, height: 36, padding: 0, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "1px solid var(--border)", borderRadius: 6, background: "var(--bg-hover)", color: "var(--text-muted)", cursor: canNavigateUp ? "pointer" : "default", opacity: canNavigateUp ? 1 : 0.45 }}>
        ...
        <div className="directory-picker-list" style={{ flex: 1, minHeight: 0, overflow: "auto", padding: "8px 10px" }}>
          {loading ? (
            ...
            <div style={{ padding: 8, color: "var(--text-dim)", fontSize: 11, lineHeight: 1.5 }}>
              Directory browsing is disabled on this server.<br />
              Type a full path above, then press &quot;Select this folder&quot;.
            </div>
          ) : (
            <>
              {shortcuts.length > 0 && (
                <div style={{ marginBottom: 6, paddingBottom: 4, borderBottom: "1px solid var(--border)" }}>
                  {shortcuts.map((shortcut) => (
                    <button
                      key={shortcut.path}
                      className="directory-picker-entry directory-picker-shortcut"
                      type="button"
                      onClick={() => void navigateTo(shortcut.path)}
                      title={shortcut.path}
                      style={{ width: "100%", minHeight: 30, display: "flex", alignItems: "center", gap: 7, padding: "5px 8px", border: 0, borderRadius: 5, background: shortcut.path === currentPath ? "var(--bg-selected)" : "none", color: "var(--text)", cursor: "pointer", textAlign: "left", fontFamily: "var(--font-mono)", fontSize: 11 }}
                    >
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }} aria-hidden="true">
                        <path d="M12 2l2.4 5.4 5.6.6-4.2 3.9 1.2 5.6-5-2.9-5 2.9 1.2-5.6L4 8l5.6-.6z" />
                      </svg>
                      <span>{shortcut.label}</span>
                      <span style={{ marginLeft: "auto", color: "var(--text-dim)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "55%" }}>{shortcut.path}</span>
                    </button>
                  ))}
                </div>
              )}
              {drives !== null ? (
                <>
                  {drives.length > 0 ? (
                    ...
                    <div style={{ padding: 8, color: "var(--text-dim)", fontSize: 11 }}>No drives found</div>
                  )}
                </>
        ...
        <div className="directory-picker-footer" style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 10, flexShrink: 0, padding: "10px 18px", borderTop: "1px solid var(--border)" }}>
          ...
          <button
            className="directory-picker-action"
            type="button"
            onClick={() => onSelect(manualMode ? pathInput.trim() : currentPath)}
            disabled={!canSelect}
            title={hasUncommittedPath ? "Press Go to open the typed path before selecting it" : "Select the current directory"}
