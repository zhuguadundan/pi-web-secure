...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onRunningSessionIdsChange }: Props) {
  ...
  const commitCustomPath = useCallback(async (candidate?: string) => {
    ...
    try {
      ...
      // Keep the resolved path so reopening the picker starts there.
      setCustomPathValue(data.cwd ?? path);
  ...
  // Web PWA: open the server-side directory picker modal. Desktop shell:
  // prefer the native selectDirectory bridge, falling back to the modal.
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      // Note: the custom-path DirectoryPicker is portaled to document.body and
      // manages its own dismissal (backdrop click, Escape, Cancel) — it must
      // not be closed from this handler, or every click inside it unmounts it.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      {/* Session list */}
      <SessionSearch
        open={Boolean(sessionSearch.trim())}
        query={sessionSearch}
