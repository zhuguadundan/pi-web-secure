...
test("rejects responses without usable model ids", () => {
  assert.deepEqual(parseDiscoveredModels({ error: { message: "nope" } }), []);
  assert.deepEqual(parseDiscoveredModels({ data: [null, 42, {}] }), []);
  assert.deepEqual(parseDiscoveredModels("not-a-list"), []);
});
