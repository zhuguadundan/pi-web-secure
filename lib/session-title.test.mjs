...
function systemMessage(content = "system") {
  return { role: "system", content, timestamp: 0 };
}

/**
 * Pi 0.87 carries the system prompt and tool declarations inside the transcript
 * instead of a separate `context.systemPrompt`, so a live source agent starts
 * with a `system` message. The mock keeps the roles the title flow observes.
 */
function createTitleSourceAgent(messages) {
  let sourceReplyFinished = false;
  const sourceAgent = {
    providerRoles: undefined,
    state: {
      systemPrompt: "system",
    ...
    streamFunction: (_model, context) => {
      ...
      sourceAgent.providerRoles = context.messages.map((message) => message.role);
...
test("waits for the source reply before sending the title prompt", async () => {
  const sourceAgent = createTitleSourceAgent([
    systemMessage(),
    { role: "user", content: "Implement auto name", timestamp: 1 },
  ]);

  const result = await generateSessionTitle({ agent: sourceAgent });

  assert.equal(result.title, "Wait for Complete Agent Reply");
  assert.deepEqual(sourceAgent.providerRoles, ["system", "user", "assistant", "user"]);
});

test("sends the source system prompt exactly once for both transcript shapes", async () => {
  const withSystem = createTitleSourceAgent([
    systemMessage(),
    { role: "user", content: "Implement auto name", timestamp: 1 },
  ]);
  await generateSessionTitle({ agent: withSystem });
  assert.equal(withSystem.providerRoles.filter((role) => role === "system").length, 1);

  // Sessions recorded before Pi 0.87 have no leading system message; the title
  // agent must still send one, and only one.
  const legacy = createTitleSourceAgent([
    { role: "user", content: "Implement auto name", timestamp: 1 },
  ]);
  const legacyResult = await generateSessionTitle({ agent: legacy });
  assert.equal(legacyResult.title, "Wait for Complete Agent Reply");
  assert.deepEqual(legacy.providerRoles, ["system", "user", "assistant", "user"]);
});
