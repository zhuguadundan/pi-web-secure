...
// POST /api/models-config/discover  body: { providerName, provider }
// Fetches the model list from a configured provider so the UI can offer
// one-click import into ~/.pi/agent/models.json. Set
// PI_WEB_DISABLE_MODEL_DISCOVERY=1 to disable outbound provider queries.
...
export async function POST(req: Request) {
  if (process.env.PI_WEB_DISABLE_MODEL_DISCOVERY === "1") {
    return NextResponse.json({ error: "Model discovery is disabled on this server (PI_WEB_DISABLE_MODEL_DISCOVERY=1)" }, { status: 403 });
  }
